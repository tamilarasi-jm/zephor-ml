...
def get_clip_embedding(image_path):
    ...
    with torch.no_grad():
        # This returns a BaseModelOutputWithPooling object
        outputs = clip_model.get_image_features(
            **inputs
        )

        # Extract the actual tensor from the pooler_output attribute
        # If get_image_features returns a tensor directly in your version,
        # this safely checks and extracts it.
    ...
    # Now you can safely normalize the PyTorch tensor
    embedding = embedding / embedding.norm(
        dim=-1,
        keepdim=True
    )
...
def generate_embeddings():
    ...
    if not os.path.exists(REFERENCE_ROOT):
        ...
        print(
            f"Folder not found: {REFERENCE_ROOT}"
        )
        return
